(function () {
  ...
  function renderDetailPage(idea) {
    ...
    return '<a class="back-link" href="#/' + idea.status + '"><i data-lucide="arrow-left"></i>返回' + STATUS_LABELS[idea.status] + '</a>' +
      ...
      '<section class="editor-section"><div class="editor-section-head"><span>02</span><div><h2>把它往前推一步</h2><p>下一步应该能在 30 分钟内开始。</p></div></div><div class="field-group"><label class="field-label" for="detailNextAction">下一步动作 <span>*</span></label><input class="text-input input-large" id="detailNextAction" name="nextAction" value="' + escapeHTML(idea.nextAction) + '" required placeholder="一个具体动作" /></div><div class="field-group"><label class="field-label" for="detailFinishLine">完成线</label><textarea class="text-input" id="detailFinishLine" name="finishLine" rows="3">' + escapeHTML(idea.finishLine) + '</textarea></div></section></div>' +
